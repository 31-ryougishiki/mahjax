...
# Boolean sentinels - in eager mode use plain Python bool
FALSE = False
TRUE = True

# Pre-built tensors for frequent use.
# In eager mode these are small tensors; for-loops or indexing into them is fine.
TILE_RANGE = torch.arange(NUM_TILE_TYPES, dtype=torch.int32)
ZERO_MASK_1D = torch.zeros(Action.NUM_ACTION, dtype=torch.bool)
ZERO_MASK_2D = torch.zeros((NUM_PLAYERS, Action.NUM_ACTION), dtype=torch.bool)

FIRST_DRAW_IDX = 135 - 13 * 4

DORA_ARRAY = torch.tensor(
    [
        1, 2, 3, 4, 5, 6, 7, 8, 0,
